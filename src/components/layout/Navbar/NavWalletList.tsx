...
const NavWalletList: React.FC<Props> = ({}) => {
    ...
    return (
        <div className="">
            ...
            <h3 className="px-3 text-sm font-medium text-gray-300">
                Wallets
            </h3>
            {data && (
            <ul>
                {data.map(w => (
                    <Link key={w.id} href={`/wallets/${w.id}`}>
                        <a className="group flex items-center rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-50 hover:text-gray-900">
